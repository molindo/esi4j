...
public class DummyIntegration {
	...
	public static void main(final String[] args) {
		...
		final Esi4J esi4j = new DefaultEsi4J(ImmutableSettings.settingsBuilder().put("esi4j.client.type", "node")
		...
		final Tweet t1 = new Tweet(1, "sfussenegger", "esi4j rocks #elasticsearch");
		final Tweet t4 = new Tweet(3, "msparer", "esi4j really rocks");

		final Integer id = (Integer) index.index(t1).actionGet().getId();
		System.out.println("indexed: " + id);
		index.index(t4).actionGet().getId();

		index.refresh();

		final Tweet t2 = (Tweet) index.get(Tweet.class, id).actionGet().getObject();
		System.out.println(t1.equals(t2));

		final Tweet none = (Tweet) index.get(Tweet.class, 4711).actionGet().getObject();
		System.out.println(none == null);

		final List<Tweet> tweets = index.multiGet(Tweet.class, Arrays.asList(new Integer[] { 1, 2, 3 })).actionGet()
		...
		final Esi4JDummyModule module = new Esi4JDummyModule().setData(Tweet.class, Lists.newArrayList(t1));
		new SimpleRebuildProcessor().rebuild((InternalIndex) index, module.startRebuildSession(Tweet.class));

		final Tweet t3 = (Tweet) index.get(Tweet.class, id).actionGet().getObject();
